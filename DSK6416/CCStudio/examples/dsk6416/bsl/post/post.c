 ...
 *  ======== post.c ========
 *
 *  The Power On Self Test (POST) is a small program that uses the 6416 DSK
 ...
 *    6      McBSP2 in loopback mode
 *    7      Internal DMA transfer
 *    8      Codec test
 *    9      LED and timer test
 *
 ...
 *  dsk6416_aic23.h, dsk6416_led.h and dsk6416_flash.h because it uses
...
void TEST_sleep(Int16 sleeptime)
{
    ...
    /* Configure based on DSP frequency */
    timerCfg1.prd = 75000;
    if (DSK6416_getVersion() == 2)
        timerCfg1.prd = 90000;
    if (DSK6416_getVersion() == 3)
        timerCfg1.prd = 125000;
...
Int16 TEST_intMem()
{
    ...
    /* Check internal memory (byte 0x20000 to byte 0x100000) */
    status |= MEM_test(0x20000, 0xE0000, 0);
    status |= MEM_test(0x20000, 0xE0000, 1);
...
Int16 TEST_flashID()
{
    ...
    /* Reset flash */
    *((Uint8 *)DSK6416_FLASH_BASE) = 0xf0;
 
    /* Configure to read manufacturer ID */
    *((Uint8 *)DSK6416_FLASH_CTLAAA) = 0xaa;
    *((Uint8 *)DSK6416_FLASH_CTL555) = 0x55;
    *((Uint8 *)DSK6416_FLASH_CTLAAA) = 0x90; 
    
    /* Insert small delay for device to respond */
    for (i = 0; i < 10; i++);

    /* Read IDs */
    MfgId = *((Uint8 *)DSK6416_FLASH_BASE);
    DevId = *((Uint8 *)DSK6416_FLASH_BASE + 2);

    /* Reset flash */
    *((Uint8 *)DSK6416_FLASH_BASE) = 0xf0;

    /* Check IDs */
    if ((MfgId != 0x0001) || (DevId != 0xb9))
        return 1;
#endif
...
Int16 TEST_mcbsp2()
{
    /* Do an internal loopback test on McBSP2 */
    return TEST_mcbsp(MCBSP_DEV2, 0);
}
...
Int16 TEST_codec()
{
    ...
    for (i = 0; i < 1000; i++)
    {
        ...
        {       
            while (!DSK6416_AIC23_write(hCodec, sinetable[j]));
            while (!DSK6416_AIC23_write(hCodec, sinetable[j]));
        }
    }
...
main()
{
    ...
    TEST_execute(TEST_mcbsp2,     6, 1);  /* McBSP2 loopback */
    TEST_execute(TEST_dma,        7, 1);  /* DMA test */
    TEST_execute(TEST_codec,      8, 0);  /* Codec test */
    TEST_execute(TEST_timer,      9, 0);  /* LED and timer test */
